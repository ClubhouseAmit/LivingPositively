...
void main() {
  group('languageName', () {
    ...
    test('unknown code passes through', () => expect(languageName('xx'), 'xx'));
  });
  ...
  group('getDirectionOfText', () {
    test(
      'Hebrew text returns rtl',
      () => expect(getDirectionOfText('שלום'), 'rtl'),
    );
    test(
      'English text returns ltr',
      () => expect(getDirectionOfText('hello'), 'ltr'),
    );
    test(
      'mixed text with Hebrew returns rtl',
      () => expect(getDirectionOfText('hello שלום'), 'rtl'),
    );
    test(
      'digits-only text defaults to ltr',
      () => expect(getDirectionOfText('12345'), 'ltr'),
    );
    test(
      'empty string defaults to ltr',
      () => expect(getDirectionOfText(''), 'ltr'),
    );
    test(
      'punctuation-only defaults to ltr',
      () => expect(getDirectionOfText('!!!'), 'ltr'),
    );
  });
}
