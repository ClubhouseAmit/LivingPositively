...
class _FailFirstLegacyCanceller {
...
void main() {
  ...
  testWidgets('cancel removes the saved schedule only after server success', (
    ...
    NotificationRepository.forService(user.service).setPreference(
      'default',
  ...
  test(
    ...
    () async {
      ...
      await NotificationRepository.forService(
        user.service,
      ).setPreference('default', existing);
      memory.notificationPreferenceWritesToFail = 1;
      final operations = <String>[];
      var mutationVersion = 0;
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      final canceller = _FailFirstLegacyCanceller();
      ...
      await expectLater(
        _onPlatform(
          ...
          () => FcmScheduledNotificationService.migrateLegacyDefaultReminder(
            ...
            idTokenProvider: () async => 'token-123',
            post: post,
            legacyNotificationCanceller: canceller.cancel,
          ),
        ),
      ...
      expect(canceller.cancelledIds, [815, 815]);
      expect(memory.stored['fcmDefaultReminderMigrated'], isTrue);
    },
  );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      NotificationRepository.forService(
        user.service,
      ).setPreference('default', previousPreference);
      await pumpUser(tester);
      var cancelRequests = 0;
      var registerRequests = 0;
      ...
      expect(
        NotificationRepository.forService(
          user.service,
        ).getPreference('default')?.hour,
        8,
      );
      expect(
        NotificationRepository.forService(
          user.service,
        ).getPreference('default')?.minute,
        15,
      );
      expect(registerRequests, 1);
      expect(memory.stored['fcmDefaultReminderMigrated'], isTrue);
