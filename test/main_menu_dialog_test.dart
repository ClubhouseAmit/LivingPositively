...
void main() {
  ...
  testWidgets('Share button includes Hebrew app URL for Hebrew locale', (
    ...
    final payload = await _sharePayloadFromMenu(tester, locale: locale);

    expect(
      payload['text'],
      equals(_expectedShareText(locale, 'https://hebsite.livepositively.club')),
    );
  ...
  testWidgets('Share button includes English app URL for English locale', (
    ...
    expect(
      payload['text'],
      equals(_expectedShareText(locale, 'https://engsite.livepositively.club')),
    );
    expect(payload['subject'], equals(_shareSubject));
  });
}
