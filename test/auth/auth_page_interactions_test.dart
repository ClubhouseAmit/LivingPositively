...
void main() {
  ...
  group('AuthPage', () {
    ...
    testWidgets(
      'authentication success should start legacy reminder migration',
      (
        tester,
      ) async {
        final userInformation = UserInformation();
        ...
        await (loginForm.onSuccess as Future<void> Function(User))(
          firebaseUser,
        );
    ...
    testWidgets(
      'notification auth pops while its FCM refresh is still pending',
      (
        tester,
      ) async {
        final userInformation = UserInformation();
        ...
        try {
          ...
          final loginForm =
              tester
                  ...
                  as dynamic;
          final completion =
              (loginForm.onSuccess as Future<void> Function(User))(
                firebaseUser,
              );
          ...
          expect(find.byType(AuthPage), findsNothing);
          expect(
            find.byKey(const Key('open-notification-auth')),
            findsOneWidget,
          );
          expect(userInformation.loggedIn, isTrue);
    ...
    testWidgets('signup should continue when the display-name update fails', (
      ...
      tester
          .widget<Button>(
            find.widgetWithText(Button, 'Create Account'),
          )
          .onPressed!();
      await tester.pump();
      await tester.pump();
    ...
    testWidgets('should report email sign-up failures', (tester) async {
      ...
      await tester.enterText(fields.at(2), 'secret');
      await tester.enterText(fields.at(3), 'secret');
      await tester.tap(find.widgetWithText(Button, 'Create Account'));
      await tester.pumpAndSettle();
...
void _expectNoSocialSection() {
  expect(find.text('or'), findsNothing);
  expect(find.text('Continue with Google'), findsNothing);
  expect(find.text('Continue with Apple'), findsNothing);
}

void _expectGoogleOnly() {
  expect(find.text('or'), findsOneWidget);
  expect(find.text('Continue with Google'), findsOneWidget);
  expect(find.text('Continue with Apple'), findsNothing);
}

void _expectAppleOnly() {
  expect(find.text('or'), findsOneWidget);
  expect(find.text('Continue with Google'), findsNothing);
