...
void main() {
  test(
    ...
    () async {
      final result = await _runGate([
        _lcov('lib/main.dart', hitLines: 1, totalLines: 1),
        _lcov(
          'lib/features/wellness_tools/ui/player.dart',
          hitLines: 1,
          totalLines: 1,
        ),
        _lcov(
  ...
  test('coverage gate rejects every configured file below its floor', () async {
    final result = await _runGate([
      _lcov('lib/main.dart', hitLines: 1, totalLines: 2),
      _lcov(
        'lib/features/wellness_tools/ui/player.dart',
        hitLines: 1,
        totalLines: 2,
    ...
    expect(
      result.stderr,
      contains('lib/features/wellness_tools/ui/player.dart'),
    );
    expect(result.stderr, contains('lib/util/async/logger_service.dart'));
  });
...
String _lcov(String path, {required int hitLines, required int totalLines}) {
