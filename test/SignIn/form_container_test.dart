...
void main() {
  ...
  testWidgets('FormContainer accepts text via controller', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(
        FormContainer(controller: controller, fieldKey: const Key('input')),
      ),
    );
  ...
  testWidgets('FormContainer hides text initially when isPasswordField=true', (
    ...
    await tester.pumpWidget(
      _wrap(FormContainer(isPasswordField: true, fieldKey: const Key('pw'))),
    );
    // Eye-off icon present indicates obscured state
    expect(find.byIcon(Icons.visibility_off), findsOneWidget);
  });

  testWidgets('FormContainer toggles visibility on suffix tap', (tester) async {
    await tester.pumpWidget(
      _wrap(FormContainer(isPasswordField: true, fieldKey: const Key('pw'))),
    );
    expect(find.byIcon(Icons.visibility_off), findsOneWidget);
  ...
  testWidgets('FormContainer shows no eye icon for non-password', (
    ...
    await tester.pumpWidget(
      _wrap(FormContainer(isPasswordField: false, hintText: 'username')),
    );
  ...
  testWidgets('FormContainer does not obscure when isPasswordField=false', (
    ...
    await tester.pumpWidget(
      _wrap(
        FormContainer(isPasswordField: false, fieldKey: const Key('plain')),
      ),
    );
