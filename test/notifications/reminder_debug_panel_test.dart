...
UserInformation _userWithReminder({String? localeName}) {
  final user = UserInformation(gender: 'male', localeName: localeName ?? '');
  NotificationRepository.forService(user.service).restorePreferences(const {
...
void main() {
  ...
  group('ReminderDebugPanel', () {
        ...
        (tester) async {
          await _onPlatform(platform, () async {
            await pumpWithProviders(
              ...
              userInformation: _userWithReminder(),
    ...
    testWidgets('Reschedule now registers the existing FCM schedule on iOS', (
      ...
      await _onIos(() async {
        ...
        await pumpWithProviders(
          tester,
          const Scaffold(body: ReminderDebugPanel()),
          userInformation: _userWithReminder(localeName: 'en'),
          surfaceSize: const Size(800, 1400),
        );
        await tester.pumpAndSettle(const Duration(milliseconds: 200));
        await tester.tap(find.byType(ExpansionTile));
    ...
    testWidgets('Clear history button rebuilds without throwing', (
      tester,
    ) async {
      await _onIos(() async {
        await pumpWithProviders(
