...
const backendWorkflowPath = '.github/workflows/_notification-backend.yml';
...
function deploymentWorkflowBlock(source) {
  ...
  const jobMatches = [...source.matchAll(/^  notification-backend:[ \t]*$/gm)];
  if (jobMatches.length !== 1) return undefined;
  const start = jobMatches[0].index;
  const remainder = source.slice(start + jobMatches[0][0].length);
  const nextJob = /^  [a-z][a-z0-9-]*:[ \t]*$/gm.exec(remainder);
  const block = source.slice(start, nextJob === null
    ? source.length : start + jobMatches[0][0].length + nextJob.index);
  if (!/^    uses:[ \t]+\.\/\.github\/workflows\/_notification-backend\.yml[ \t]*$/m.test(block)) {
    return undefined;
  }
...
function workflowDeploymentChanged(baseCommitSha, workingDirectory) {
  ...
  let previousBackendWorkflow;
  try {
    previousWorkflow = runGit(
      ['show', `${baseCommitSha}:${workflowPath}`], workingDirectory,
    );
    previousBackendWorkflow = runGit(
      ['show', `${baseCommitSha}:${backendWorkflowPath}`], workingDirectory,
    );
  } catch {
    return true;
  }
  let currentWorkflow;
  let currentBackendWorkflow;
  try {
    currentWorkflow = readFileSync(join(workingDirectory, workflowPath), 'utf8');
    currentBackendWorkflow = readFileSync(
      join(workingDirectory, backendWorkflowPath), 'utf8',
    );
  } catch {
    return true;
  }
  ...
  return previousBlock === undefined || currentBlock === undefined ||
    ...
    previousBackendWorkflow !== currentBackendWorkflow;
}
