...
function repository(t) {
  ...
  commit('.github/workflows/main.yml',
    `jobs:\n${workflowStart}\n  backend: base\n${workflowEnd}\n`);
  commit('.github/workflows/_notification-backend.yml',
    'jobs:\n  release:\n    runs-on: ubuntu-latest\n');
  return { workingDirectory, git, commit };
}

function reusableWorkflowRepository(t) {
  const repo = repository(t);
  for (const path of [
    '.github/workflows/main.yml',
    '.github/workflows/_notification-backend.yml',
  ]) {
...
test('actual reusable workflow skips unrelated pushes and tracks backend changes', async (t) => {
  await t.test('unrelated push', (t) => {
    const repo = reusableWorkflowRepository(t);
    const baseline = repo.git('rev-parse', 'HEAD');
    const current = repo.commit('docs/readme.md');
    assert.equal(plan(repo, baseline, current).functionsRequired, false);
  });

  await t.test('unrelated main.yml job change', (t) => {
    const repo = reusableWorkflowRepository(t);
    const baseline = repo.git('rev-parse', 'HEAD');
    const path = '.github/workflows/main.yml';
    const source = readFileSync(join(repo.workingDirectory, path), 'utf8');
    const changed = source.replace(
      '  quality:\n', '  quality:\n    # Unrelated quality comment\n',
    );
    assert.notEqual(changed, source);
    const current = repo.commit(path, changed);
    assert.equal(plan(repo, baseline, current).functionsRequired, false);
  });

  await t.test('notification-backend job change', (t) => {
    const repo = reusableWorkflowRepository(t);
    const baseline = repo.git('rev-parse', 'HEAD');
    const path = '.github/workflows/main.yml';
    const source = readFileSync(join(repo.workingDirectory, path), 'utf8');
    const changed = source.replace(
      ...
      '      coverage-result: ${{ needs.backend-quality.result }}',
    );
    assert.notEqual(changed, source);
    const current = repo.commit(path, changed);
    assert.equal(plan(repo, baseline, current).functionsRequired, true);
  });

  await t.test('missing notification-backend job fails closed', (t) => {
    const repo = reusableWorkflowRepository(t);
    const baseline = repo.git('rev-parse', 'HEAD');
    const path = '.github/workflows/main.yml';
    const source = readFileSync(join(repo.workingDirectory, path), 'utf8');
    const changed = source.replace(
      '  notification-backend:\n', '  notification-backend-disabled:\n',
    );
    assert.notEqual(changed, source);
    const current = repo.commit(path, changed);
    assert.equal(plan(repo, baseline, current).functionsRequired, true);
  ...
  await t.test('called backend workflow change', (t) => {
    const repo = reusableWorkflowRepository(t);
    const baseline = repo.git('rev-parse', 'HEAD');
    const path = '.github/workflows/_notification-backend.yml';
    const source = readFileSync(join(repo.workingDirectory, path), 'utf8');
    const current = repo.commit(path, `${source}\n# release changed\n`);
    assert.equal(plan(repo, baseline, current).functionsRequired, true);
  });
});
